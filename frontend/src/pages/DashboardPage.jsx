...
export const DashboardPage = () => {
  ...
  const isElevated = emotion.driverState === 'Stressed' || emotion.stressScore >= 75;
  const isFatigued = emotion.driverState === 'Fatigued';

  // Calculate dynamic risk score based on driver emotion
  const dynamicRiskScore = isElevated ? 64 : isFatigued ? 48 : 22;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card
          ...
          badge={
            <StatusBadge status={isElevated ? 'high-stress' : 'nominal'} size="sm">
              Risk {dynamicRiskScore}%
            </StatusBadge>
          }
        >
          <div className="space-y-3">
            <div className="flex items-baseline justify-between">
              <span className="text-3xl font-semibold tracking-tight text-zinc-950 dark:text-white font-tabular">
                {dynamicRiskScore}<span className="text-sm font-normal text-zinc-400">%</span>
              </span>
            ...
            <div className="space-y-1.5 text-xs">
              <div className="flex justify-between">
                ...
                <span className="text-zinc-900 dark:text-zinc-200 font-medium font-tabular">
                  {isElevated ? '84% (Turn 4)' : '18% (Nominal)'}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-zinc-500">Apex Miss Delta:</span>
                <span className="text-zinc-900 dark:text-zinc-200 font-medium font-tabular">
                  {isElevated ? '+0.34s loss' : '+0.05s optimal'}
                </span>
              </div>
            </div>
        ...
        {/* 3. Session & Pace Summary */}
        <Card
          title="Session & Pace Summary"
          subtitle="Silverstone Grand Prix"
          badge={<Badge variant="outline" size="sm">Lap {currentAnalysis?.lap || 18}/52</Badge>}
        >
        ...
        >
          <div className="space-y-2.5 text-xs">
            <div className="p-2.5 rounded-md bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-950 text-xs leading-relaxed flex items-start gap-2 shadow-2xs">
              ...
              <span>{currentAnalysis?.recommendation?.action || 'Enforce radio silence through Sector 2 high-G corners.'}</span>
            </div>
            
            <div className="flex items-center justify-between text-xs pt-1">
              <span className="text-zinc-500">
                Pit Window: <strong>{currentAnalysis?.recommendation?.pitWindow || 'Lap 21'}</strong>
              </span>
              <button
                type="button"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Left Column (2 Cols): Live Radio Activity & Lap Chart */}
        <div className="lg:col-span-2 space-y-6">
          
          {/* Recent Radio Activity Feed (Connected to Live RadioContext History) */}
          <Card
                ...
                <div className="relative">
                  ...
                  <div className="space-y-0.5">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[11px] text-zinc-400 font-tabular">14:15:30</span>
                    </div>
                    <p className="text-zinc-500 leading-relaxed text-xs">
                      Comms held during 4.8G braking into Stowe
                    </p>
                  </div>
                </div>
            ...
            <div className="space-y-3">
              ...
              <div className="pt-2">
                <Button variant="primary" size="sm" className="w-full gap-2">
                  ...
                  Initiate Pit Protocol ({currentAnalysis?.recommendation?.pitWindow || 'Lap 21'})
                </Button>
              </div>
            </div>
          </Card>
